    ...
    <div class="w-full bg-white shadow flex flex-col my-4 p-6">
        ...
        @foreach ($categories as $category)
            <a href="{{ route('category', $category) }}"
                class="text-semibold block py-2 px-3 rounded {{ request('category') && request('category')->slug == $category->slug ? 'bg-blue-600 text-white' : 'hover:bg-blue-200' }}">{{ $category->title }}
                ({{ $category->total }})</a>
        @endforeach
    </div>
